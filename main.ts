 ...
 */
function guessAccount(itemName: string): string {
  ...
  if (transportWords.some(w => s.includes(w))) return "旅費交通費";

  // 通信
  const commWords = ["通信", "スマホ", "携帯", "回線", "wifi", "インターネット"];
  if (commWords.some(w => s.includes(w))) return "通信費";

  // 仕入っぽい（食品・飲料）
  const foodWords = ["パン", "おにぎり", "弁当", "コーヒー", "お茶", "牛乳", "ヨーグルト", "菓子", "チョコ", "アイス", "ジュース", "米", "肉", "魚", "野菜"];
  if (foodWords.some(w => s.includes(w))) return "仕入高";

  // 消耗品っぽい
  const dailyWords = ["袋", "マスク", "洗剤", "ティッシュ", "トイレット", "ラップ", "電池", "歯ブラシ", "シャンプー", "タオル", "スポンジ", "ゴミ袋"];
  if (dailyWords.some(w => s.includes(w))) return "消耗品費";
 ...
 */
function processImage(base64: string) {
